...
import { z } from 'zod';
import { Shield, Mail, Lock, User, Briefcase, Eye, EyeOff, Loader2 } from 'lucide-react';
...
type SetupFormValues = z.infer<typeof initialSetupSchema>;
...
export const SetupPage: React.FC = () => {
  ...
  const onSubmit = async (data: SetupFormValues) => {
    ...
    try {
      await apiClient.post('/auth/initial-setup', {
      ...
      setErrorMessage(err.response?.data?.detail || 'An error occurred during root setup. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 overflow-hidden bg-surface-900">
      ...
      <div className="w-full max-w-md glass bg-surface-800/80 shadow-[0_0_50px_rgba(14,165,233,0.1)] border-white/10 p-8 flex flex-col relative z-10 transition-all duration-300">
        ...
        <div className="absolute top-0 left-10 right-10 h-[2px] bg-gradient-to-r from-transparent via-neon-blue-400 to-transparent opacity-80" />
        ...
        <div className="flex flex-col items-center mb-6 text-center">
          ...
          <p className="text-gray-400 text-xs max-w-xs leading-relaxed">
            Establish the root administrator profile to unlock and initialize the Question Bank Portal.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full glass-input pl-10 pr-4 py-2.5 text-sm ${
                  errors.full_name ? 'border-neon-red-500/50 focus:border-neon-red-500' : ''
                }`}
              />
            </div>
            {errors.full_name && (
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text"
                {...register('department')}
                placeholder="e.g. IT & Academic Systems"
                className={`w-full glass-input pl-10 pr-4 py-2.5 text-sm ${
                  errors.department ? 'border-neon-red-500/50 focus:border-neon-red-500' : ''
                }`}
              />
            </div>
            {errors.department && (
          ...
          {/* Warning / Lockout Notice */}
          <div className="p-3 rounded-lg bg-yellow-500/5 border border-yellow-500/10 text-[10px] text-yellow-500/80 leading-relaxed">
            ⚠️ <strong>Setup Lockout:</strong> Initial setup is locked immediately after execution. Subsequent admin registrations will require delegation by this account.
          </div>
