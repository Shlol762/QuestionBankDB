...
const AllowedGradesManager: React.FC = () => {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState('');
  const [editing, setEditing] = useState<AllowedGrade | null>(null);
  const [gradeName, setGradeName] = useState('');
  const [note, setNote] = useState('');
  const [active, setActive] = useState(true);

  const { data, isLoading } = useQuery({
    queryKey: ['allowed-grades', search],
    queryFn: async () => {
      const params = new URLSearchParams();
      params.append('limit', '300');
      if (search) params.append('search', search);
      const res = await client.get(`/allowed-grades/?${params.toString()}`);
      return res.data;
    },
  });

  const items: AllowedGrade[] = data?.items || [];

  const createMutation = useMutation({
    mutationFn: (payload: { grade_name: string; recommendation_note?: string; is_active: boolean }) =>
      client.post('/allowed-grades/', payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allowed-grades'] });
      setGradeName('');
      setNote('');
      setActive(true);
      toast.success('Allowed grade added');
    },
    onError: (err: unknown) => {
      const error = err as { response?: { data?: { detail?: string } } };
      toast.error(error.response?.data?.detail || 'Failed to add allowed grade');
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: { grade_name?: string; recommendation_note?: string; is_active?: boolean } }) =>
      client.patch(`/allowed-grades/${id}`, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allowed-grades'] });
      setEditing(null);
      setGradeName('');
      setNote('');
      setActive(true);
      toast.success('Allowed grade updated');
    },
    onError: (err: unknown) => {
      const error = err as { response?: { data?: { detail?: string } } };
      toast.error(error.response?.data?.detail || 'Failed to update allowed grade');
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => client.delete(`/allowed-grades/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allowed-grades'] });
      toast.success('Allowed grade removed');
    },
    onError: (err: unknown) => {
      const error = err as { response?: { data?: { detail?: string } } };
      toast.error(error.response?.data?.detail || 'Failed to remove allowed grade');
    },
  });

  const isBusy = createMutation.isPending || updateMutation.isPending;

  const title = useMemo(() => (editing ? 'Edit Allowed Grade' : 'Create Allowed Grade'), [editing]);

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    const cleanName = gradeName.trim();
    if (!cleanName) {
    ...
    if (editing) {
      updateMutation.mutate({ id: editing.allowed_grade_id, payload });
      return;
    }
  ...
  const startEdit = (item: AllowedGrade) => {
    setEditing(item);
    ...
    <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
      <div className="xl:col-span-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
        ...
        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-1">Grade Name</label>
            <input
              value={gradeName}
      ...
      <div className="xl:col-span-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
        ...
        <div className="space-y-3 max-h-[560px] overflow-y-auto pr-1">
          {isLoading ? (
            ...
            <p className="py-16 text-center text-sm text-gray-500">No allowed grades found</p>
          ) : (
            items.map((item) => (
              <div key={item.allowed_grade_id} className="border border-gray-100 dark:border-gray-700 rounded-2xl p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-black text-sm text-gray-900 dark:text-white">{item.grade_name}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => deleteMutation.mutate(item.allowed_grade_id)} className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-gray-500 hover:text-red-600">
...
export default AllowedGradesManager;
